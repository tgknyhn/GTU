...
\solution
\newline
\centerline{\includegraphics[scale=.4]{question1_hasse.png}}
\newpage
\problem{2: Relations}{15}
Answer these questions for the poset ($\{\{$1$\}$, $\{$2$\}$, $\{$4$\}$, $\{$1, 2$\}$, $\{$1, 4$\}$, $\{$2, 4$\}$, $\{$3, 4$\}$, $\{$1, 3, 4$\}$, $\{$2, 3, 4$\}\}$, $\subseteq$).
\newline
\newline
\centerline{\includegraphics[scale=.4]{question2_hasse.png}}
\subproblem{a} Find the maximal elements.
\newline
\newline (Maximal elements are the elements that have no related upper element on Hasse diagram.)
\newline
\solution Maximal Elements: \{ \{1,2\}, \{1,3,4\}, \{2,3,4\} \} 
\newline
\subproblem{b} Find the minimal elements.
\newline
\newline (Minimal elements are the elements that have no related element below them on Hasse diagram.)
\newline
\solution Minimal elements: \{ \{1\}, \{2\}, \{4\} \}
\newline
\subproblem{c} Is there a greatest element?
\newline
\solution There is no greatest element because there are more than one maximal element.
\newline
\subproblem{d} Find all upper bounds of $\{\{$2$\}$, $\{$4$\}\}$.
\newline
\solution 2 and 4 are both subsets of 2 elements. And these are : \{2,4\} and \{2,3,4\}
\newline
\subproblem{e} Find the least upper bound of $\{\{$2$\}$, $\{$4$\}\}$, if it exists.
\newline
\solution As we go down on the Hasse diagram we find least upper bound. And since \{2,3,4\} is above of \{2,3\}, we say that element \{2,3\} is the least upper bound of \{\{$2$\}$, $\{$4$\}\}
\newline
\subproblem{f} Find all lower bounds of $\{\{$1, 3, 4$\}$, $\{$2, 3, 4$\}\}$.
\newline
\solution There are total of 2 elements both related to \{$1, 3, 4$\} and \{$2, 3, 4$\}. If we look into same connected element, we find first(greatest lower bound) element as \{3,4\} and one step below we find second(least lower bound) element as \{4\}.
\newline
\subproblem{h} Find the greatest lower bound of $\{\{$1, 3, 4$\}$, $\{$2, 3, 4$\}\}$,
if it exists.
\newline
\solution It exist because there is only one element below which related to both \{$1, 3, 4$\} and \{$2, 3, 4$\}. And it is \{3,4\}.
\newline
