...
enum cellstates { empty = '.', user1 = 'x', user2 = 'o', bigO = 'O', bigX = 'X'};
...
void saveGame	   	 (char board[MAX_SIZE][MAX_SIZE], const string move, const string name, bool & user, int & sizeOfBoard, bool & pveORpvp);
void loadGame	     (char board[MAX_SIZE][MAX_SIZE], const string move, const string name, bool & user, int & sizeOfBoard, bool & pveORpvp);

//computer moves
void askComputer     (char board[MAX_SIZE][MAX_SIZE], int & row, char & column, int & sizeOfBoard);
void findLastMove    (char board[MAX_SIZE][MAX_SIZE], char letter, int * routeX, int * routeY, int & currY, int & sizeOfBoard); 
...
void initialize_game(int & size, char & player_or_computer)
{
	...
	} while (  (size > 12 || size < 6) || (player_or_computer != '1' && player_or_computer != '0')  );
}
...
// opens a file named as "name" string then fills the file with necessary information about game(sizeofboard, board state, etc.)
void saveGame(char board[MAX_SIZE][MAX_SIZE], const string move, const string name, bool & user, int & sizeOfBoard, bool & pveORpvp)
{
	ofstream input;
	input.open(name);

	input << sizeOfBoard << endl << user << endl << pveORpvp << endl;
	for(int i=1; i<=sizeOfBoard; i++)
		for(int j=1; j<=sizeOfBoard; j++)
			input << board[i][j];
	input << endl;
...
void loadGame(char board[MAX_SIZE][MAX_SIZE], const string move, const string name, bool & user, int & sizeOfBoard, bool & pveORpvp)
{
	...
	if(size > 12 || size < 6)
	{
		cerr << endl << "The game named \"" << name << "\" doesn't exist.\n" << endl;
		changeUser(user);
		return;
	}
	else
	{
		output >> user >> pveORpvp;
		sizeOfBoard = size;
	}
...
bool askUser(char board[MAX_SIZE][MAX_SIZE], int & row, char & column, bool & user, int & sizeOfBoard, bool & pveORpvp)
{
	...
	if(move == "SAVE")
	{
		saveGame(board, move, name, user, sizeOfBoard, pveORpvp);
		return false;
	}
	else if(move == "LOAD")
	{
		loadGame(board, move, name, user, sizeOfBoard, pveORpvp);
		return false;
	}
	else
	{
		...
		auto x = column - 64; // ex: a == 97(ASCII value)
		auto y = row;
...
void searchRouteU1(char board[MAX_SIZE][MAX_SIZE], int * routeX, int * routeY, int currIndex, bool * flag, int & sizeOfBoard)
{
	...
	{
		for(int i=1; i<=6; i++)
			if(checkNeighbors(board, routeX, routeY, currIndex, i, user1) == true)
				searchRouteU1 (board, routeX, routeY, currIndex+1, flag, sizeOfBoard);		
	}
...
void searchRouteU2(char board[MAX_SIZE][MAX_SIZE], int * routeX, int * routeY, int currIndex, bool * flag, int & sizeOfBoard, bool user)
{
	...
	{
		for(int i=1; i<=6; i++)
			if(checkNeighbors(board, routeX, routeY, currIndex, i, user2) == true)
				searchRouteU2 (board, routeX, routeY, currIndex+1, flag, sizeOfBoard, user);		
	}
}
...
void findLastMove(char board[MAX_SIZE][MAX_SIZE], char letter, int * routeX, int * routeY, int & currX, int & currY, int & sizeO)
{
...
void askComputer(char board[MAX_SIZE][MAX_SIZE], int & row, char & column, int & sizeOfBoard)
{
	...
	findLastMove(board, user1, routeX, routeY, xb, yb, sizeOfBoard);
	...
	if(isNeighborsEmpty(board, x, y, empty_neighbors) == true)
		...
		goStraightO(board, row, column, sizeOfBoard);
...
void convertToUpper(char board[MAX_SIZE][MAX_SIZE], int * routeX, int * routeY, bool user)
{
	if(user == 0)
		for(int i=1; i<MAX_POSSIBLE_MOVE; i++)
			board[routeY[i]][routeX[i]] = bigX;

	else if(user == 1)
		for(int i=1; i<MAX_POSSIBLE_MOVE; i++)
			board[routeY[i]][routeX[i]] = bigO;
}
