...
void ctrlcla(int argc, char* argv[]) {
	...
	if(strcmp(argv[1], "-i") == 0 && strcmp(argv[3], "-n") == 0) {
		...
		strcpy(path_to_sem_milk, argv[4]);
		strcpy(path_to_sem_flour, argv[5]);
		strcpy(path_to_sem_walnuts, argv[6]);
		strcpy(path_to_sem_sugar, argv[7]);
		strcpy(path_to_sem_pusherLock, argv[8]);
		strcpy(path_to_sem_wholesaler, argv[9]);
        for(int i=0; i<CHEF_COUNT; i++)
		    strcpy(path_to_sem_chefs[i], argv[i+9]);
	}
	else
...
void openSemaphores() {
    sem_wholesaler = sem_open(path_to_sem_wholesaler, SEM_FLAG_CE, SEM_PERMS, 0); sem_unlink(path_to_sem_wholesaler);
    sem_milk       = sem_open(path_to_sem_milk, SEM_FLAG_CE, SEM_PERMS, 0);       sem_unlink(path_to_sem_milk);
    sem_flour      = sem_open(path_to_sem_flour, SEM_FLAG_CE, SEM_PERMS, 0);      sem_unlink(path_to_sem_flour);
    sem_walnuts    = sem_open(path_to_sem_walnuts, SEM_FLAG_CE, SEM_PERMS, 0);    sem_unlink(path_to_sem_walnuts);
    sem_sugar      = sem_open(path_to_sem_sugar, SEM_FLAG_CE, SEM_PERMS, 0);      sem_unlink(path_to_sem_sugar);
    sem_pusherLock = sem_open(path_to_sem_pusherLock, SEM_FLAG_CE, SEM_PERMS, 1); sem_unlink(path_to_sem_pusherLock);
    for(int i=0; i<CHEF_COUNT; i++) {
        sem_chefs[i] = sem_open(path_to_sem_chefs[i], SEM_FLAG_CE, SEM_PERMS, 0); sem_unlink(path_to_sem_chefs[i]);
    }
}
